/*
 * force_uniphy2_sgmiiplus.c - Force UCG-Fiber / UXG-Fiber uniphy2 (eth5) to SGMII+ 2.5G
 *
 ...
 * loop accepts SGMII+ as the correct mode. Port 6 (eth5) is excluded from
 ...
 * other ports (LAN + eth6 SFP+ trunk) normally.
 *
 * Target: UCG-Fiber / UXG-Fiber, IPQ9574, kernel 5.4.213-ui-ipq9574
 * Module: qca-ssdk.ko must be loaded
 *
 * BUILD:   make -C /lib/modules/$(uname -r)/build M=$(pwd) modules
 * LOAD:    insmod force_uniphy2_sgmiiplus.ko
 * UNLOAD:  rmmod force_uniphy2_sgmiiplus  (reverts to SGMII 1G)
 * VERIFY:  cat /sys/kernel/debug/clk/uniphy2_gcc_tx_clk/clk_rate
...
MODULE_DESCRIPTION("Force UCG-Fiber / UXG-Fiber uniphy2 to SGMII+ 2.5G");
...
#define UNIPHY_INDEX 2
#define SSDK_PORT_ID 6
#define DEV_ID 0
...
static int __init force_sgmiiplus_u2_init(void)
{
	int ret;
	unsigned int bmp;
	...
	if (!uniphy_mode_set || !port_iface_mode_set || !dt_global_set_mac_mode ||
		...
		pr_err("force_sgmiiplus_u2: kallsyms lookup failed\n");
		return -ENOENT;
	}

	pr_info("force_sgmiiplus_u2: resolved all symbols\n");
	...
	pr_info("force_sgmiiplus_u2: port bitmap 0x%x -> 0x%x (port %d excluded)\n",
		orig_port_bmp, bmp, SSDK_PORT_ID);

	ret = uniphy_mode_set(DEV_ID, UNIPHY_INDEX, SSDK_UNIPHY_SGMIIPLUS);
	if (ret != 0) {
		pr_err("force_sgmiiplus_u2: uniphy_mode_set failed: %d\n", ret);
		port_bmp_set(DEV_ID, orig_port_bmp);
		ssdk_mac_sw_sync_work_start(DEV_ID);
		return ret;
	}
	pr_info("force_sgmiiplus_u2: uniphy%d set to SGMII+ 2.5G\n", UNIPHY_INDEX);
	...
	msleep(1000);
	ssdk_mac_sw_sync_work_start(DEV_ID);
	pr_info("force_sgmiiplus_u2: loop restarted (port %d excluded)\n", SSDK_PORT_ID);
	...
	/* Update SSDK SFP PHY speed cache so ethtool/sysfs/UDAPI report 2500 */
	{
		...
		if (priv_addr) {
			...
			*sp = SPEED_2500;
			*dp = DUPLEX_FULL;
			pr_info("force_sgmiiplus_u2: speed cache %u -> %u\n",
		 ...
		 * immediately -- doesn't block the dsmp event loop. */
		{
			static char *argv[] = {
				"/bin/sh", "-c",
				"sleep 2 && ip link set eth5 alias x && ip link set eth5 alias ''",
				NULL
			};
		...
		pr_info("force_sgmiiplus_u2: speed reporting updated\n");
	}

	return 0;
}

static void __exit force_sgmiiplus_u2_exit(void)
{
	if (!uniphy_mode_set)
		return;
	...
	if (priv_addr) {
		...
		*sp = orig_speed;
		*dp = orig_duplex;
		pr_info("force_sgmiiplus_u2: speed cache restored to %u\n", orig_speed);
	}
	...
	ssdk_mac_sw_sync_work_start(DEV_ID);
	pr_info("force_sgmiiplus_u2: reverted, loop restarted with full bitmap 0x%x\n",
		orig_port_bmp);
}

module_init(force_sgmiiplus_u2_init);
module_exit(force_sgmiiplus_u2_exit);
