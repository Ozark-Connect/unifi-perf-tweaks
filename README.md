...
- **MongoDB SSD:** See [docs/recovery.md](docs/recovery.md) for the full rollback guide. It covers three scenarios, because the right path depends on *why* you're rolling back: **Path A** (live-migrate back to eMMC with current data — use when the SSD is healthy and you just want off the offload, ~95% of rollbacks), **Path B** (mongorestore from the latest `07-mongodb-ssd-backup.sh` dump — use when the SSD or its data is broken), or **Path C** (nuclear fallback on the pre-deploy eMMC snapshot — stale, only use if Paths A and B both fail). In every path, stop `unifi-mongodb.service` *before* `umount` — the unmount fails with EBUSY while mongod has files open on the bind mount, and a plain `systemctl stop unifi` does not stop mongod (it's owned by a separate unit).
