...
- **UniFi OS 5.1.26 EA** - verified statically (no gateway on 5.1.26 yet). The kernel is unchanged and `qca-ssdk.ko` is code-identical to the verified 5.1.19/5.1.21 SSDK (`.text` byte-identical, all SGMII+ symbols and cache offsets intact — the md5 differs only in build-id and embedded build-provenance strings), and all boot-tweak 06/07/10/15 userland dependencies are present in the firmware rootfs. The live SGMII+ load/unload and "tweaks in effect" checks are deferred to a Network Optimizer field deploy; see [docs/compat-5.1.26.md](docs/compat-5.1.26.md).
- **UniFi OS 5.1.28 EA** - verified statically on the bench (no gateway on 5.1.28 yet). The kernel is unchanged and `qca-ssdk.ko` is **byte-identical to the verified 5.1.26 SSDK** (`cmp` clean; therefore code-identical to 5.1.19/5.1.21 with all SGMII+ symbols and cache offsets intact), and all boot-tweak 06/07/10/15 userland dependencies are present in the firmware rootfs. Live checks deferred to a later deploy; see [docs/compat-5.1.28.md](docs/compat-5.1.28.md).
