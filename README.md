...
- **UniFi OS 6.0.5 EA** - **field-confirmed working on the UXG-Fiber** (SGMII+ module + tweaks 10+15, live on a production gateway carrying real WAN traffic at 2.5G with zero interface errors). This is a **major release**, not a maintenance patch: the userland rebases from Debian 11 to **Debian 13 (trixie)**, moving Python 3.9 → 3.13, systemd 247 → 257, and syslog-ng 3.x → 4.8.1. Both affected scripts survived unchanged because they depend on stable surfaces (the `python3` symlink and the `ustd.statusdb.sdb_client` import path for `15`; the `conf.d` layout and route syntax for `10`) - and both correctly re-applied after the upgrade reset stock config. `qca-ssdk.ko` **changes for the first time since 5.1.19**, but only as a GCC 10 → 14 recompile riding along with the Debian rebase: all 10 SGMII+ symbols are present, the uniphy/sgmii symbol family is unchanged at 114, `.data`/`.bss` sizes are identical, the entire symbol delta is compiler clone-name churn, and the `0x690`/`0x6d0` cache offsets are confirmed by byte-identical `sfp_read_status` instruction encodings. vermagic is unchanged, so the prebuilt modules load without a rebuild. Note this round is UXG-Fiber-only because **6.0.5 EA has not been released for the UCG-Fiber** - unusually, the UXG-Fiber got it first, reversing the normal order. **Tweaks 06+07 are therefore unverified on 6.0.5** - the UXG-Fiber ships no MongoDB or Network app at all, so those need a UCG-Fiber on 6.0.5, which is not yet possible; see [docs/compat-6.0.5.md](docs/compat-6.0.5.md).
...
> **SGMII+ (`19` / `20`):** Promoted from Testing to Stable — the modules have been running continuously on production UCG-Fiber and UXG-Fiber gateways for a couple months, across every UniFi OS release from 5.1.15 through 6.0.5 (including the Debian 13 rebase in 6.0.x, which required no module rebuild). Deploy **one script at a time**: loading both modules simultaneously is not supported (see the [port bitmap caveat](docs/sfp-sgmiiplus.md#port-bitmap-exclusion)).
